---
title: "Theme"
description: "Every theme key in @crosmos/graph, with defaults, and the CSS variables that drive popovers and the zoom indicator."
icon: "palette"
---
...
Override on `.cg-root` or `.cg-popover` to retarget popovers and the zoom indicator. Defaults follow a shadcn-style palette and the included `.dark` block follows the `next-themes` class convention.

| Variable         | Default (light)              | Purpose                           |
| ---------------- | ---------------------------- | --------------------------------- |
| `--cg-bg`        | `oklch(0.96 0.005 99.4974)`  | Popover background.               |
| `--cg-fg`        | `oklch(0.1 0.02 99.4974)`    | Popover text.                     |
| `--cg-muted-fg`  | `oklch(0.35 0.02 99.4974)`   | Row labels and zoom indicator.    |
...
Map straight to your existing design tokens:
