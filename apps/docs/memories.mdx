...
Every memory is either `private` (the default) or `org`. Org-visible memories are readable by every member of the organization; private memories are readable only by their owner. A source's visibility cascades to the memories and edges extracted from it.
...
For endpoint details, use the API Reference tab.
