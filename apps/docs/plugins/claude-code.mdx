...
The Crosmos Claude Code plugin adds persistent project context to Claude Code. It recalls relevant context when a session starts, captures completed exchanges through lifecycle hooks, and adds `/crosmos-recall` and `/crosmos-save` skills for on-demand lookups and explicit notes.

Use this plugin when you want Claude Code to remember project decisions, preferences, and prior work without managing memory by hand. MCP is optional. Use it when you need callable Crosmos memory tools in another MCP client. See the [MCP overview](/mcp/overview).
...
<Card title="GitHub repository" icon="github" href="https://github.com/crosmos-labs/claudecode-crosmos">
