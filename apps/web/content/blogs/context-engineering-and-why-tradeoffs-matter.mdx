...
description: "A practical look at context engineering, memory tradeoffs, ranking, retrieval, and why agent systems need better context layers."
...
Have been working on the context engineering problem for a while and just wanted to share my thoughts on it and why we are building [@crosmoslabs](https://x.com/crosmoslabs).
...
The exponential curve by itself is a correct way to think about forgetting memories. But the metrics used in the curve by themselves aren't enough to decide if a memory should be forgotten or not. We need **access patterns** for a more "personalized" behavior. When was a memory retrieval and how many times a memory was retrieved. But not all memories are same. Some are preferences. Some are just events happened in the past. Some are just stated facts about the world. Blindly treating them "equally" for forgetting is not the right approach for a truly **cognitive memory system.** The forgetting itself has to be aware of the what kind of memory it is. I see many people trying to "over-complicate" the forgetting part. But I feel you just need the right types and the right weights for each type of memory to decide what to forget. An equation itself can't really solve it.

## SUMMARIES / CONSOLIDATION

This is probably the most misunderstood part of any memory system. Usually the basic mental model is. - Get a set of semantically / type based similar memories and consolidate them into a topic / general memory. This itself is correct by definition as human brain creates a general summary of a person / entity based on the attributes or "memories" the user has about the entity. But the part where everyone gets it wrong is. They let those consolidated memories take part in recall with the specific memories of the entities. Why is this "wrong" by design? It's because after a while the system will be filled with so many consolidated memories that after a certain threshold of memories stored, it'll just return generic facts about entities. Now this is wrong because when you recall something, you usually want to recall "specific" details. The generic details about an entity are just it's traits. So why still consolidate memories? Treat them as an **evidence, not just another type of memory.**

We observed this after using Crosmos heavily ourselves for a long time. It all comes down to "giving the LLM the right corpus to reason over" The consolidated evidence should be retrieved "with" the specific facts, not just them. By that I mean, don't let generic statements fight with specific facts to dump them down.

## Bottom line

These are the principles [@crosmoslabs](https://x.com/crosmoslabs) is built on. We are actively working on our system, making it better, and making sure it's not just something that can perform "perfect" on a benchmark but also is **usable** to our users and caters to the enterprise needs. When a team can share the same memory, that's where the magic happen. Centralized cognition.
