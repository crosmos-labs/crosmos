...
description: "Why memory quality depends on representation, structure, and reasoning over context, not just maximizing recall into a prompt."
...
Why? Because understanding contradictions between memories and states at ingestion layer means you're basically trusting the LLM to be correct about all the time.
...
But reasoning about contradictions at ingestion layer can cause, **destructive updates**, **contamination of the knowledge graph or wrong assumptions**.
...
At [@crosmoslabs](https://x.com/crosmoslabs) we decided to have a similar approach. you can checkout our docs [https://docs.crosmos.dev/](https://docs.crosmos.dev/) to understand the approach we took after iterating over multiple architectures.

What we learned is, memory should be auditable, state-aware and shouldn't lose it's provenance.
...
It's all about **tradeoffs** . But the tradeoffs you make decides if your architecture will scale or not.
