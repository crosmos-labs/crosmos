...
Crosmos is shown under both its own grader and an independent one. Competitor scores are from HydraDB's evaluation [^hydradb].

<div className="not-prose mt-6 border-l-2 border-muted-foreground pl-4 text-sm text-muted-foreground">We're actively benchmarking across a range of models, so the figures above are a current snapshot and may shift as that work continues.</div>
