...
note: "Draft v0. Accuracy numbers are from LongMemEval-s on gpt-5-mini, graded by gpt-5-mini."
---
...
memory 99.7% of the time, answers around 91% of questions correctly on a small model, and runs a
deterministic search that returns the same results for the same query.
...
That determinism is deliberate. A growing number of memory systems put language-model calls
...
Crosmos puts the answer in the top 10 results 99.7% of the time, measured without telling search
what kind of question it is. That is the foundation everything downstream stands on: if the
answer is in the returned set, a capable model can use it; if it is not, nothing afterward can
recover it.

### 5.2 Accuracy

Answering with gpt-5-mini, Crosmos scores 91% on LongMemEval, and 90.8% with gpt-4o as the
grader. By category (gpt-4o):
...
Among production memory systems on LongMemEval-s:
...
Competitor scores are as reported in HydraDB's evaluation [^hydradb].
